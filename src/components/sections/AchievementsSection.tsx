    ...
    <section className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {t.achievements.items.map((achievement, index) => (
          <div
            key={index}
            className="group relative rounded-lg bg-card border border-border overflow-hidden hover:border-primary/50 transition-all duration-300 animate-fade-in"
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden bg-neutral-900">
              ...
              {/* Hover overlay */}
              <div className={`absolute inset-0 flex items-center justify-center bg-black/60 transition-all duration-300 ${hoverIndex === index ? "opacity-100" : "opacity-0"}`}>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {achievement.tags.map((tag, tagIndex) => (
                  <span key={tagIndex} className="px-2.5 py-1 text-xs rounded-md bg-primary/10 text-primary border border-primary/20 font-medium">
                    {tag}
                  </span>
                ))}
              </div>
