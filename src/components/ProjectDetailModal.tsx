...
import { useProjectDetailModal } from "@/hooks/useProjectSection";

const ProjectDetailModal = ({ project, onClose }: Props) => {
  const { t, projectModal, onCloseModal } = useProjectDetailModal(project, onClose);
      ...
      <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-6xl bg-card rounded-2xl shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <div className="grid md:grid-cols-2 md:h-[90vh] md:overflow-hidden">
          <div className="relative flex items-center justify-center">
            <img src={projectModal.image} alt={projectModal.title} className="w-full h-auto max-h-full object-contain" />
          </div>

          <div className="p-6 space-y-5 md:overflow-y-auto">
            <div>
              <h2 className="text-xl font-bold">{projectModal.title}</h2>
              <p className="text-muted-foreground">{projectModal.description}</p>
            </div>

            <div>
              <h3 className="font-semibold mb-2">{t.projects.techstack}</h3>
              <div className="flex flex-wrap gap-2">
                {projectModal.tech?.map((tech) => (
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{projectModal.problem}</p>
            </div>

            <div>
              <h3 className="font-semibold">{t.projects.roletitle}</h3>
              <p className="text-sm text-muted-foreground">{projectModal.role}</p>
            </div>

            <div>
              <h3 className="font-semibold">{t.projects.impacttitle}</h3>
              <p className="text-sm text-muted-foreground">{projectModal.impact}</p>
            </div>

            <div>
              <h3 className="font-semibold">{t.projects.featurestitle}</h3>
              <ul className="mt-3 space-y-2 pl-2">
                {projectModal.features.map((featured) => (
            ...
            <div className="flex gap-3 pt-4 flex-wrap">
              {projectModal.demolink && (
                <a href={projectModal.demolink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition">
                  <ExternalLink className="w-4 h-4" />
                  {t.projects.demo}
                </a>
              )}

              {projectModal.sourcelink && (
                <a href={projectModal.sourcelink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-lg border border-border hover:bg-muted transition">
